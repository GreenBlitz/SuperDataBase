...
def main():
    gen = pic_generator(10, 4, 80, 250, 1280, 720) #Example
    rand_pic_np, ball_set = gen.__next__() #take the first thing the generator yielded
...
def get_random_pic(max_balls: int, min_diam: int, max_diam: int, bg_width: int, bg_height: int):
    ...
    :return: a numpy array representing a random background from "Demo-BGSet" resized to bg_width x bg_height with ball_count = 0...max_balls random balls from "Demo-BallSet"
                ...
                when the array is sorted in accending order by the sum of it's center coordinates.
    """
    bg_file = random.choice(os.listdir("Demo-BGSet")) #take a random file from dir "Demo-BGSet"
    ...
    for i in range(len(ball_list)):
        ball_file = random.choice(os.listdir("Demo-BallSet")) #take a random file from dir "Demo-BallSet"
...
def randomize_balls(max_balls: int, min_diam: int, max_diam: int, bg_width: int, bg_height: int):
    ...
    for i in range(ball_count):
        legal = False #legal means it does not intersect with any other balls already saved
        while not legal:
            tup = create_ball(bg_width, bg_height, max_diam, min_diam) #the randomization part
            legal = is_ball_legal(tup_list, tup) #the varifying part
        tup_list.append(tup)
    return tup_list
...
def is_ball_legal(tup_list: list, tup: tuple):
    ...
    for i in tup_list:
        if (((i[0] + i[2] / 2.0) - (tup[0] + tup[2] / 2.0)) * ((i[0] + i[2] / 2.0) - (tup[0] + tup[2] / 2.0)) +
            ...
            ((tup[2] + i[2]) / 2.0 + 1) * ((tup[2] + i[2]) / 2.0 + 1)): #if does not intersect
            return False
    return True
...
def circle_copy_stupid(ball_arr: np.ndarray, bg_arr: np.ndarray, x_top_left: int, y_top_left: int, diameter: int):  # diameter equals to width
    ...
    for x_counter in range(diameter + 1): 
        for y_counter in range(diameter + 1):
            if (r * r) >= (x_center - x_counter) ** 2 + (y_center - y_counter) ** 2: #if in circle
                bg_arr[y_top_left + y_counter, x_top_left + x_counter] = ball_arr[y_counter, x_counter] #print the pixel in the right place
    return bg_arr
